   ...
   // private $_link;
    ...
    private function  _getPageNumbers()
    {
        ...
        if($this->textNav&&($this->currentPage > 1)){
            $html .= '<li><a href="'.$this->link .'?'.$this->sort.'current='.($this->currentPage-1).'"';
        ...
        if($last < $this->range) {
            $start      = ( ( $this->currentPage - $this->itemsPerPage) > 0 ) ? $this->currentPage - $this->itemsPerPage : 1;
            $end        = ( ( $this->currentPage + $this->itemsPerPage ) < $last ) ? $this->currentPage + $this->itemsPerPage : $last;

        }elseif($this->total > $this->range){
        ...
        for($i = $start; $i <= $end; $i++){
            if($i==$this->currentPage) $html .= '<li class="active" ><a href="javascript:void(0)">';
            else $html .= '<li><a href="'.$this->link .'?'.$this->sort.'current='.$i.'" >';
            $html .= $i.'</a></li>';
        }
    ...
    private function  _getItemSelect()
    {
        ...
        return "<span class=\"paginate\">".$this->_navigation['ipp']."</span>
            <select class=\"paginate\" onchange=\"window.location='$this->link.'?'.$this->sort.'current=1&item='+this[this.selectedIndex].value;return false\">$items</select>\n";
    }
}